...
static void process_image(const void *p, int size)
{
	...
	i++;
        ...
        // UDP_send (sock_udp, p, size);        
	if (UDP_send (sock_udp, p, size) > 0)
	printf ("send frame %d, size: %d\n", i, size);
}
...
long_options[] = {
		...
		// please define variables for ip and port to use UDP
        { "address", required_argument, NULL, 'a'},
        { "port", required_argument, NULL, 'p'},
        { 0, 0, 0, 0 }
};

void* thread_WiFi_Control_TMR(void* vargs){
 	...
 	if ((duty1 = fopen("/sys/devices/ocp.3/pwm_test_P8_19.17/duty", "w")) == NULL){
  	...
  	if ((run1 = fopen("/sys/devices/ocp.3/pwm_test_P8_19.17/run", "w")) == NULL){
        ...
        while(1){
                numbytes = UDP_recv(sock_udp, buf, 99);
                if(numbytes < 4) 
                        continue;
...
// please insert the WiFi_Control_TMR.c which you wrote in lab 4
int main(int argc, char **argv)
{
	...
	//  please write your own script using UDP_new_socket, UDP_socket_init
		...
		//  please write your own script of case 'a' for ip
                case 'a':
                        strcpy(str_ip, optarg);
                        break;
		//  please write your own script of case 'p' for port
                case 'p':
                        port = atoi(optarg);
                        break;
		...
		//  please write your own script using UDP_set_client, pthread_create
        UDP_set_client(sock_udp, port, str_ip);
        printf("port:%d\n", port);
        printf("ip:%s\n", str_ip);
        pthread_create(&tid, NULL, thread_WiFi_Control_TMR, NULL);
	...
	frame_count = 0;
		...
		//  please write your own script using pthread_join
